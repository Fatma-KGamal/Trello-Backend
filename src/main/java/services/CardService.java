...
public class CardService {
	...
	public void addComment(long cardId, Card commentCard) {
		...
		notifyUser("Comment added to " + card.getCardName() + " card successfully ");
	}
	...
	public void addDescription(long cardId, Card descriptionCard) {
		...
		entityManager.merge(card);
		// notify user when description is added
		notifyUser("Description added to " + card.getCardName() + " card successfully ");
	}
	...
	public List<Card> getAllCards() {
		if (User.isCurrentUser() == false) {
			...
			TypedQuery<Card> query = entityManager.createQuery("SELECT c from Card c LEFT JOIN FETCH c.cardList ",
					Card.class);
