...
//	                entityManager.merge(user);
					TypedQuery<Board> query = entityManager.createQuery("SELECT b from Board b WHERE b.boardId = :id",
							Board.class);
...
//					entityManager.flush();
		...
		if (User.isCurrentUser() == false) {
			...
			if (teamLeader.isAdmin() == false) {
				...
				TypedQuery<Board> query = entityManager
						.createQuery("SELECT b from Board b LEFT JOIN FETCH b.users u WHERE u.id = :id", Board.class);
	...
	public List<Board> getAllBoards() {
		...
		if (User.isCurrentUser() == false) {
			...
			TypedQuery<Board> query = entityManager
					.createQuery("SELECT DISTINCT b from Board b LEFT JOIN FETCH b.users ", Board.class);
